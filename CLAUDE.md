...
- After 2 unsuccessful fix attempts: AUTOMATICALLY check FIRST if problem is caused by Cursor browser limitations
- If Cursor browser issue found: RECOMMEND testing in regular browser, incognito mode, or disabling Cursor extension
- If NOT a Cursor browser issue: AUTOMATICALLY search Stack Overflow FIRST for solutions
- If Stack Overflow has solutions: AUTOMATICALLY try ALL applicable Stack Overflow solutions (not just 2)
- If Stack Overflow has no solutions: AUTOMATICALLY search GitHub for top 2 solutions, test both
- If all Stack Overflow/GitHub solutions fail: AUTOMATICALLY search for 4 more solutions from web/GitHub/forums/chatboards, implement and test all 4 one by one
- If all solutions fail: AUTOMATICALLY propose creative new solutions that haven't been tried
